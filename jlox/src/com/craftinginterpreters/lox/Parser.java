...
public class Parser {
    ...
    private Stmt classDeclaration() {
        ...
        List<Stmt.Function> static_methods = new ArrayList<>();
        List<Stmt.Function> getters = new ArrayList<>();
        while (!check(RIGHT_BRACE) && !isAtEnd()) {
            if (match(CLASS)) {
                static_methods.add(function("static method"));
            } else {
                if (match(IDENTIFIER) && peek().type != LEFT_PAREN) {
                    back();
        ...
        return new Stmt.Class(name, superclass, methods, static_methods, getters);
    }
